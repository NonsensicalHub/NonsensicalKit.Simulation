...
{
    /// <summary>
    /// 五轴串链机械臂：轴点父子链配置连杆，由 <see cref="RobotArm5Clip"/> 点位链表 IK 求解。
    /// 轴数固定为 5。设备构型（Rest 时世界轴）：J1=Y，J2/J3/J4=X（互相平行且始终水平），J5=Y。
    /// 末端朝下：保持第 5 轴在目标正上方（J5→TCP 对齐世界下方）；俯仰由第 4 轴完成（轴始终水平）；J5 仅扭转。
    /// </summary>
    [DisallowMultipleComponent]
    [AddComponentMenu("ScriptAnimation/五轴机械臂 (RobotArm5Anim)")]
    public class RobotArm5Anim : ScriptAnimActor
    {
        public const int JointCountFixed = 5;
        public const int MinJointCount = JointCountFixed;
        public const int DefaultJointCount = JointCountFixed;
        public const int J5Index = 4;
        /// <summary>朝下约束帧：第 5 轴（在目标正上方）。</summary>
        public const int TipDownJointIndexFixed = 4;
        /// <summary>朝下俯仰关节：第 4 轴（水平轴，一次可转到）。</summary>
        public const int WristPitchJointIndexFixed = 3;
        ...
        [Tooltip("工具接近轴（末端本地）。完整朝向约束时使用；「末端保持向下」改用第 5 轴→TCP")]
        [InspectorLabel("工具接近轴")]
        [SerializeField] private Vector3 m_toolApproachLocal = Vector3.up;

        [Header("末端姿态约束")]
        [Tooltip("保持第 5 轴在目标正上方：J5→TCP 对齐世界下方。俯仰由第 4 轴（始终水平）完成；第 5 轴扭转与朝下同帧求解。")]
        ...
        [Header("调试")]
        [Tooltip("开启后 Timeline 预览禁用 IK 缓存，每帧完整重解并输出 [RobotArm5 DBG] 日志")]
        ...
        /// <summary>腕部俯仰起始关节（含）：第 4 轴，用于朝下转到。</summary>
        public int WristStartIndex => WristPitchJointIndexFixed;

        /// <summary>朝下约束帧：第 5 轴（在目标正上方，J5→TCP）。</summary>
        ...
        public void ApplyClipDefaults(RobotArm5ClipData data)
        {
            if (data == null)
                return;
        ...
        {
            Debug.Log(RobotArm5Ik.BuildPoseDebugReport(this), this);
        }
        ...
        public bool Validate(out List<string> errors, out List<string> warnings)
        {
            ...
            if (n == JointCountFixed)
            {
                Vector3 j2 = GetAxisLocal(1);
                Vector3 j3 = GetAxisLocal(2);
                Vector3 j4 = GetAxisLocal(3);
                if (Vector3.Dot(j2, j3) < 0.85f || Vector3.Dot(j3, j4) < 0.85f)
                    warnings.Add("J2/J3/J4 本地旋转轴应近似平行（设备为同一水平轴，Rest 时世界 X）。");

                Vector3 j1 = GetAxisLocal(0);
                Vector3 j5 = GetAxisLocal(4);
                if (Mathf.Abs(Vector3.Dot(j1.normalized, Vector3.up)) < 0.85f)
                    warnings.Add("J1 本地轴建议为 Y（Rest 时世界竖直）。");
                if (Mathf.Abs(Vector3.Dot(j5.normalized, Vector3.up)) < 0.85f)
                    warnings.Add("J5 本地轴建议为 Y（末端扭转）。");
            }
        ...
        public bool TrySolveIkToTarget()
        {
            ...
            bool ok = RobotArm5Ik.SolveToTarget(
                this,
                m_ikTarget.position,
                angles,
                angles,
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (m_joints != null)
            {
                Gizmos.color = new Color(0.95f, 0.55f, 0.22f, 0.95f);
                for (int i = 0; i < n; i++)
                {
                ...
                if (m_keepTipDown)
                {
                    ...
                    {
                        actual.Normalize();
                        Gizmos.color = Vector3.Dot(actual, DownWorldAxis) >= RobotArm5Ik.TipDownAcceptDot
            ...
            RobotArm5Ik.ForwardPositions(this, angles, out Vector3[] jointPos, out Vector3 tip);
            if (jointPos == null)
                return;

            Gizmos.color = new Color(0.95f, 0.6f, 0.2f, 0.35f);
