    ...
    public static class ConveyorPathPlanner
    {
            ...
            WarehouseJob job = null)
        {
                ...
                && preferredOutfeedPortIndex < topology.OutfeedNodeIndices.Count)
            {
                ...
                if (IsOutfeedAvailableForAssignedJob(topology, bindings, outfeedReservationCounts, preferredOutfeedPortIndex)
                    && TryFindPathForStrategy(
                        ...
                        pickupPointIndex, outfeedNode, routePlanTime, job, out var path)
            ...
            for (var port = 0; port < topology.OutfeedNodeIndices.Count; port++)
            {
                ...
                if (!TryFindPathForStrategy(
                        topology, bindings, strategy, reservations,
                        pickupPointIndex, outfeedNode, routePlanTime, job, out var candidatePath)
                    || candidatePath == null
                    || candidatePath.Count < 2)
                {
                    continue;
        ...
        private static bool TryFindPathForStrategy(
            ...
            out List<int> pathNodeIndices)
        {
            if (ConveyorProcessStationUtility.HasRequiredProcessTags(job))
            {
                return TryFindPathViaRequiredProcessStations(
                    topology,
                    bindings,
                    strategy,
                    reservations,
            ...
            pathNodeIndices = null;
            if (!topology.TryFindShortestPathByEffectiveTransit(
                    fromNodeIndex,
                    toNodeIndex,
                    nodeIndex => ConveyorVerticalTransferUtility.GetNodeArrivalExtraSeconds(
                        topology,
                        nodeIndex,
                        bindings,
                        job),
        ...
        /// <summary>为每个必需加工标签选取可达站点并拼接最短路。</summary>
        private static bool TryFindPathViaRequiredProcessStations(
            ConveyorMapTopology topology,
            IWarehouseSimulationBindings bindings,
            WarehouseSimStrategyProfile strategy,
            ReservationTable reservations,
        ...
        {
            pathNodeIndices = null;
            if (topology == null || job?.RequiredProcessTags == null)
            {
                return false;
            }

            var waypoints = new List<int>();
            var current = fromNodeIndex;
            for (var t = 0; t < job.RequiredProcessTags.Length; t++)
            {
                ...
                var candidates = ConveyorProcessStationUtility.GetDwellStationsWithTag(topology, tag);
                ...
                var bestStation = -1;
                var bestScore = float.MaxValue;
                List<int> bestLeg = null;
                for (var c = 0; c < candidates.Count; c++)
                {
                    var station = candidates[c];
                    if (!TryFindDirectPathForStrategy(
                            topology,
                            bindings,
                            strategy,
                            reservations,
                            ...
                            station,
                            atTime,
                            out var legToStation)
                        || legToStation == null
                        || legToStation.Count < 2)
                    {
                        continue;
                    }
                    ...
                    var score = ScorePath(topology, bindings, strategy, reservations, legToStation, atTime)
                                + ScorePath(topology, bindings, strategy, reservations, legToDest, atTime);
                    if (score >= bestScore)
                    {
                        continue;
                    }
                    ...
                    bestScore = score;
                    bestStation = station;
                    bestLeg = legToStation;
                }

                if (bestStation < 0 || bestLeg == null)
                {
                    return false;
                }

                waypoints.Add(bestStation);
                current = bestStation;
            }

            return TryStitchPath(
                topology,
                bindings,
                strategy,
                reservations,
        ...
        private static bool TryFindDirectPathForStrategy(
            ConveyorMapTopology topology,
            IWarehouseSimulationBindings bindings,
            WarehouseSimStrategyProfile strategy,
            ReservationTable reservations,
        ...
        private static bool TryStitchPath(
            ConveyorMapTopology topology,
            IWarehouseSimulationBindings bindings,
            WarehouseSimStrategyProfile strategy,
            ReservationTable reservations,
            ...
            out List<int> pathNodeIndices)
        {
            pathNodeIndices = new List<int> { fromNodeIndex };
            var current = fromNodeIndex;
            ...
            if (current == toNodeIndex)
            {
                return pathNodeIndices.Count >= 1;
            }
            ...
            if (!TryFindDirectPathForStrategy(
                ...
                || finalSegment == null
                || finalSegment.Count < 2)
            {
                pathNodeIndices = null;
                return false;
            }

            for (var i = 1; i < finalSegment.Count; i++)
            {
                pathNodeIndices.Add(finalSegment[i]);
            }

            return pathNodeIndices.Count >= 2;
        }
