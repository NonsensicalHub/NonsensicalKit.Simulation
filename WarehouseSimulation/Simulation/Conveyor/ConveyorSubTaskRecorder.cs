...
namespace NonsensicalKit.Simulation.WarehouseSimulation.Simulation
{
    ...
    internal sealed class ConveyorSubTaskRecorder
    {
            ...
            bool isFirstZoneOnPath)
        {
            ...
            // 取货点/出库口/加工站点 zone 的驶入 hop 已在来向边 flush（RecordEdgeSlotSubTasksFromChain）中记录。
            if (zone.Kind is ConveyorPathZoneKind.Pickup
                or ConveyorPathZoneKind.Outfeed
                or ConveyorPathZoneKind.ProcessStation
                or ConveyorPathZoneKind.VerticalTransfer)
            {
                return;
            }
            ...
            ref bool pathAttached)
        {
            ...
            for (var i = 0; i < chain.Count; i++)
            {
                ...
                if (zone.PathEdgeIndex == pathEdgeIndex
                    && zone.Kind is ConveyorPathZoneKind.Pickup
                        or ConveyorPathZoneKind.Outfeed
                        or ConveyorPathZoneKind.ProcessStation
                        or ConveyorPathZoneKind.VerticalTransfer)
                {
        ...
        public void RecordProcessStationService(WarehouseJob job, in ConveyorPathZone zone)
        {
            if (job == null || zone.Kind != ConveyorPathZoneKind.ProcessStation)
            {
                return;
            }

            ref var node = ref _topology.GetNode(zone.ToNodeIndex);
            if (!ConveyorProcessStationUtility.JobRequiresStationTag(job, in node))
            {
                return;
            }

            var serviceStart = zone.ArriveSimTime;
            var serviceEnd = zone.LeaveSimTime;
            if (serviceEnd <= serviceStart + 1e-9)
            {
                return;
            }
            ...
            _record(
                job,
                SimSubTaskKind.ProcessStationService,
                serviceStart,
                serviceEnd,
                job.AssignedStackerId,
                job.TargetSlot,
        ...
        public void RecordVerticalTransferMove(WarehouseJob job, in ConveyorPathZone zone)
        {
            if (job == null || zone.Kind != ConveyorPathZoneKind.VerticalTransfer)
            {
                return;
            }
            ...
            _record(
                job,
                SimSubTaskKind.VerticalTransferMove,
                serviceStart,
                serviceEnd,
                job.AssignedStackerId,
                job.TargetSlot,
