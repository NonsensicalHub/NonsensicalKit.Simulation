...
namespace NonsensicalKit.Simulation.WarehouseSimulation.Core
{
    ...
    public sealed class ConveyorMapTopology
    {
        ...
        public IReadOnlyList<int> ProcessStationNodeIndices { get; }
            ...
            Dictionary<(int from, int to), SimConveyorMapEdge> edgeData)
        {
            ...
            ProcessStationNodeIndices = processStations;
            _outEdges = outEdges;
            _inEdges = inEdges;
            _edgeData = edgeData;
            ...
            for (var i = 0; i < map.Nodes.Length; i++)
            {
                ...
                switch (node.Kind)
                {
                    ...
                    case SimConveyorNodeKind.ProcessStation:
                        processStations.Add(i);
                        if (node.ProcessMode == SimConveyorProcessMode.Dwell
                            && string.IsNullOrWhiteSpace(node.ProcessTag))
                        {
                            error = $"加工站点节点 {logicalId} 需要配置 ProcessTag（如 wrap）。";
                            return false;
                        }
        ...
        /// <summary>
        /// 按边输送时间 + 节点停留/服务时间的最短路径（不缓存；用于含提升机/加工站的路径择优）。
        /// </summary>
        public bool TryFindShortestPathByEffectiveTransit(
            int fromNodeIndex,
            int toNodeIndex,
            Func<int, float> nodeArrivalExtraSeconds,
            out List<int> path)
        {
            path = null;
            if (fromNodeIndex == toNodeIndex)
            ...
            for (var iter = 0; iter < n; iter++)
            {
                ...
                foreach (var (to, transit) in _outEdges[u])
                {
                    ...
                    var next = dist[u] + transit + nodeArrivalExtraSeconds(to);
                    if (next < dist[to])
                    {
                        dist[to] = next;
                        parent[to] = u;
        ...
        public float EstimatePathSeconds(IReadOnlyList<int> pathNodeIndices) =>
            EstimatePathSeconds(pathNodeIndices, null);

        /// <summary>估算路径总时间（边输送 + 节点停留/服务，不含 ZPA 积放等待）。</summary>
        public float EstimatePathSeconds(
            ...
            Func<int, float> nodeArrivalExtraSeconds)
        {
            ...
            for (var i = 1; i < pathNodeIndices.Count; i++)
            {
                ...
                if (nodeArrivalExtraSeconds != null)
                {
                    total += nodeArrivalExtraSeconds(pathNodeIndices[i]);
                }
            }
