...
namespace NonsensicalKit.Simulation.WarehouseSimulation.Core
{
    ...
    public sealed class SimResourcePolicyConfig : ISimResourcePolicy
    {
        ...
        [Label("加工站点默认服务时间（秒）")]
        [Tooltip("地图上加工站点 ProcessServiceSeconds ≤ 0 时使用。")]
        public float ProcessStationServiceSeconds = 30f;

        [Label("垂直提升机默认提升时间（秒）")]
        [Tooltip("地图上垂直提升机 TransferSeconds ≤ 0 时使用。")]
        public float VerticalTransferSeconds = 15f;
        ...
        float ISimResourcePolicy.ProcessStationServiceSeconds => ProcessStationServiceSeconds;
        float ISimResourcePolicy.VerticalTransferSeconds => VerticalTransferSeconds;
