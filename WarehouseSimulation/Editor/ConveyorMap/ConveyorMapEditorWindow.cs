...
namespace NonsensicalKit.Simulation.WarehouseSimulation.Editor
{
    public sealed class ConveyorMapEditorWindow : EditorWindow
    {
        ...
        private void CreateGUI()
        {
            ...
            var styleSheet = ConveyorMapGraphViewStyles.EditorStyleSheet;
            if (styleSheet != null)
            {
        ...
        private void DrawToolbar()
        {
            ...
            if (GUILayout.Button("加工站", EditorStyles.toolbarButton, GUILayout.Width(52)))
            {
                _graphView?.AddNodeOfKind(SimConveyorNodeKind.ProcessStation);
            }

            if (GUILayout.Button("提升机", EditorStyles.toolbarButton, GUILayout.Width(52)))
            {
                _graphView?.AddNodeOfKind(SimConveyorNodeKind.VerticalTransfer);
            }
        ...
        private static void DrawNodeInspector(SerializedProperty element)
        {
            ...
            switch (kind)
            {
                ...
                case SimConveyorNodeKind.ProcessStation:
                    EditorGUILayout.LabelField("加工站点参数", EditorStyles.boldLabel);
                    EditorGUILayout.HelpBox(
                        "输送线上的加工设备（如缠膜机）。加工模式为「停留加工」时，匹配标签的货物进入后等待服务时间。\n" +
                        "ProcessTag 示例：wrap（缠膜）。任务在流程计划中配置 RequiredProcessTags 后必经对应站点。", MessageType.None);
                    DrawRelativeProperty(element, "ProcessMode");
                    DrawRelativeProperty(element, "ProcessTag");
                    DrawRelativeProperty(element, "ProcessServiceSeconds");
                    break;

                case SimConveyorNodeKind.VerticalTransfer:
                    EditorGUILayout.LabelField("垂直提升机参数", EditorStyles.boldLabel);
                    EditorGUILayout.HelpBox(
                        "跨层移动节点：输送路径规划与普通节点相同，提升时间与动画可单独配置。\n" +
                        "各层节点共用 TransferGroupId 以互斥预约同一台提升机；目标层节点留空时取路径下一节点锚点。", MessageType.None);
                    DrawRelativeProperty(element, "TransferGroupId");
                    DrawRelativeProperty(element, "TransferSeconds");
                    DrawRelativeProperty(element, "TransferTargetLogicalId");
                    DrawRelativeProperty(element, "TransferMotion");
                    break;
            }

            EditorGUILayout.Space(6);
            DrawRelativeProperty(element, "MaxReservations");
        }
        ...
        internal static void RemoveSegment(WarehouseConveyorMap map, string segmentKey)
        {
            ...
            var removeReverse = ConveyorMapEditorEdgeUtility.IsBidirectionalSegment(map, from, to);
            var edges = new List<SimConveyorMapEdge>();
            foreach (var e in map.Edges)
            {
                var edgeFrom = e.FromNodeId?.Trim();
                var edgeTo = e.ToNodeId?.Trim();
                if (edgeFrom == from && edgeTo == to)
                {
                    continue;
                }

                if (removeReverse && edgeFrom == to && edgeTo == from)
                {
                    continue;
                }
