...
- **模块定位**：管理交互目标入队、切换与执行。
- **核心入口**：`InteractQueueSystem`、`TriggerInteractableObject`、`ColliderInteractExecutor`
- **使用方法**：
  1. 准备交互系统服务与交互菜单 UI。
...
- `Sample Training`：交互队列、任务推进、库存 UI 与子场景协同示例。
- `ParametricModelingShelves`：货架参数化生成、层间距调整与货位显隐示例。
